...
module.exports = async (req, { res }) => {
	...
	if (!wikiDocuments) {
		...
		res.setPageData({
			page: "wiki.ejs",
			pageTitle: `Search for "${req.query.q}" - HuskyBot Wiki`,
...
module.exports.readArticle = async (req, { res }) => {
	...
	if (!wikiDocuments) {
		...
		res.setPageData({
			page: "wiki.ejs",
			pageTitle: `${page._id} - HuskyBot Wiki`,
			pageList: wikiDocuments.map(wikiDocument => wikiDocument._id),
			mode: "page",
			title: page._id,
...
module.exports.edit = async (req, { res }) => {
	const renderPage = data => {
		res.setPageData({
			page: "wiki.ejs",
			pageTitle: `${data.title ? `Edit ${data.title}` : "New Page"} - HuskyBot Wiki`,
			mode: "edit",
			...data,
		});
...
module.exports.history = async (req, { res }) => {
	...
	if (!wikiDocuments) {
		...
		res.setPageData({
			page: "wiki.ejs",
			pageTitle: `Edit history for ${page._id} - HuskyBot Wiki`,
			pageList: wikiDocuments.map(wikiDocument => wikiDocument._id),
			mode: "history",
