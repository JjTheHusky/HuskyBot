	...
	const renderPage = async (upvotedData, serverData) => {
		...
		try {
			...
			const galleryDocuments = await Gallery.find(matchCriteria).sort({ featured: -1, points: -1, last_updated: -1 }).skip(count * (page - 1))
				.limit(count)
				.exec();
			const pageTitle = `${extensionState.charAt(0).toUpperCase() + extensionState.slice(1)} - HuskyBot Extensions`;
...
controllers.my = async (req, { res }) => {
	if (req.isAuthenticated()) {
		try {
			...
			res.setPageData({
				page: "extensions.ejs",
				pageTitle: "My HuskyBot Extensions",
				serverData: {
					id: req.user.id,
				},
...
controllers.builder = async (req, { res }) => {
	if (req.isAuthenticated()) {
		const renderPage = extensionData => {
			...
			res.setPageData({
				page: "extensions.ejs",
				pageTitle: `${extensionData.name ? `${extensionData.name} - ` : ""}HuskyBot Extension Builder`,
				activeSearchQuery: req.query.q,
				mode: "builder",
				extensionData,
			...
			switch (req.params.action) {
				...
				case "accept": {
					...
					messageOwner(galleryDocument.owner_id, {
						embed: {
							color: Colors.GREEN,
							title: `Your extension ${galleryDocument.name} has been accepted ${galleryDocument.level === "third" ? "by maintainers." : "to the HuskyBot extension gallery!"} 🎉`,
				...
				case "feature":
					if (!galleryDocument.featured) {
						messageOwner(galleryDocument.owner_id, {
							embed: {
								color: Colors.GREEN,
								title: `Your extension ${galleryDocument.name} has been featured on the HuskyBot extension gallery! 🌟`,
				...
				case "remove": {
					...
					messageOwner(galleryDocument.owner_id, {
						embed: {
							...
							title: `Your extension ${galleryDocument.name} has been ${actionString} ${galleryDocument.level === "third" ? "by maintainers" : "from the HuskyBot extension gallery"}.`,
